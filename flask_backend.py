...
import pandas as pd
from flask import Flask, request, jsonify, render_template #flask prenderà i dati dell'utente e li inserirà come input nell'algoritmo
...
def append_new_row(date, cycle_flg, df):
    ...
    if cycle_flg == 1:
        ...
        days_until, duration = predict_next_cycle(df.tail(1))
...
@app.route('/get_predicted_cycles')
def get_predicted_cycles():
    # Get the last row (you can modify this to select the relevant data)
    last_row = df.tail(1)

    # Get the predictions from the model (you can implement this)
    days_until, duration = predict_next_cycle(last_row)

    # Calculate the predicted cycle dates
    start_date = datetime.datetime.now() + datetime.timedelta(days=round(days_until[0]))
    cycle_days = [{"start": (start_date + datetime.timedelta(days=i)).strftime('%Y-%m-%d')} for i in range(round(duration[0]))]

    # Return the predicted cycle days
    return jsonify({"predicted_cycles": cycle_days})
...
def submit_feedback():
    ...
    append_new_row(date, cycle, df)
    
    return jsonify({'status': 'success', "message": "Feedback received and CSV updated."})
